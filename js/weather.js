...
$.ajax({
	...
	success : function(parsed_json) {
		...
		for (var i = 0; i < 8; i++) {
				...
				$( ".calandar_day:contains('Night')" ).remove();
		};
		...
		$('.calandar_day').hover(
			function() {
				...
				for (var key in all_description) {
					...
					if (first_3_letters === first3_3_letters) {
						weatherBox.append("<div class='description'><h2 class='d_title'>" + key + ": </h2><p class='d_txt'>" + all_description[key] + "</p></div>");
						$("description").css({
						    position: 'absolute'
						});
					} else {}	
		...
		$('.calandar_day:last-child').attr('id', 'last_day');
